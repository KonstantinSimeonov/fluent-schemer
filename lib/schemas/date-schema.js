...
export const factory = function (BaseSchema, { createError, ERROR_TYPES }) {
    ...
    return class DateSchema extends BaseSchema {
        constructor() {
            super();
            console.log('Warning: DateSchema is experimental and still under development! Use carefully in production.');
        }
        ...
        weekdayBetween(start, end) {
            if (typeof (this._startDay && this._endDay) !== 'undefined') {
                throw new Error('Cannot set an weekday between on a date schema instance twice');
            }

            if (!validateRangeBound(start) || !validateRangeBound(end)) {
                throw new Error(`Expected sane integer numbers for .weekdayBetween() method, but got ${start} and ${end}`);
            }
