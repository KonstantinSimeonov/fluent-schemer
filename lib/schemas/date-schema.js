import is from '../is';
console.warn('Warning: DateSchema is experimental and still under development! Use carefully in production.');
	...
	return class DateSchema extends BaseSchema {
		...
		dateBetween(start, end) {
			...
			if (!validateRangeBound(start) || !validateRangeBound(end)) {
				throw new RangeError(`Expected sane integer numbers for .dateBetween() method, but got ${start} and ${end}`);
			}
		...
		monthBetween(start, end) {
			...
			if (!validateRangeBound(start) || !validateRangeBound(end)) {
				throw new RangeError(`Expected sane integer numbers for .monthBetween() method, but got ${start} and ${end}`);
			}
		...
		hourBetween(start, end) {
			...
			if (!validateRangeBound(start) || !validateRangeBound(end)) {
				throw new RangeError(`Expected sane integer numbers for .hourBetween() method, but got ${start} and ${end}`);
			}
		...
		weekdayBetween(start, end) {
			if (!is.undefined(this._startDay || this._endDay)) {
				throw new Error('Cannot set an weekday between on a date schema instance twice');
			}

			if (!validateRangeBound(start) || !validateRangeBound(end)) {
				throw new RangeError(`Expected sane integer numbers for .weekdayBetween() method, but got ${start} and ${end}`);
			}
